def calculaExpressoes(string):
    if len(string) == 0:
        ...
        s_colchetes = s_parenteses[:abre] + \
            str(numeros[0]) + s_parenteses[fecha+1:]
        chaves = ''
        ...
        if s_espaco[0] != '{' or s_espaco[-1] != '}':
            s_chaves = s_colchetes[:abre] + \
                str(numeros[0]) + s_colchetes[fecha+1:]
            numeros = []
            operadores = []
            num_atual = ''
...
expressao = "3 + {100 - [4 + (15+13-7/3.5*4) - 22] * 7 }"
